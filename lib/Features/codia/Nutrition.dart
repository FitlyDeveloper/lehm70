...
class CodiaPage extends StatefulWidget {
  ...
  final String scanId;
  // Add a parameter to determine if this is a temporary view (don't save data permanently)
...
class _CodiaPage extends State<CodiaPage>
  ...
  void initState() {
    ...
    // Initialize default nutrient values first
    _initializeDefaultValues();
    
    // Process immediate nutrition data from widget if available
    if (widget.nutritionData != null && widget.nutritionData!.isNotEmpty) {
      print(
          'Processing immediate nutrition data from widget: (${widget.nutritionData!.keys.join(", ")})');
      _updateNutrientValuesFromData(widget.nutritionData!);
    }

    // Then start the async data loading process
    _initializeNutrientData();

    // After loading data is complete, update UI
  ...
  void _updateNutrientValuesFromData(Map<String, dynamic> data) {
    ...
    // FIRST process structured nested fields for vitamins, minerals, other_nutrients
    // Process structured 'vitamins' field - this is the format used by FoodCardOpen.dart for ingredients
    try {
      if (data.containsKey('vitamins') && data['vitamins'] is Map) {
        Map<String, dynamic> vitaminsData =
            Map<String, dynamic>.from(data['vitamins']);

        // Map API vitamin keys to our UI keys
        Map<String, String> apiToUiVitaminMap = {
          'vitamin_a': 'Vitamin A',
          'vitamin_d': 'Vitamin D',
          'vitamin_e': 'Vitamin E',
          'vitamin_k': 'Vitamin K',
          ...
          'folate': 'Vitamin B9', // Also handle folate as Vitamin B9
        };

        vitaminsData.forEach((apiKey, value) {
          String vitaminKey = apiToUiVitaminMap[apiKey] ??
              apiToUiVitaminMap[apiKey.toLowerCase()] ??
              apiKey;

          if (vitamins.containsKey(vitaminKey)) {
            if (value is Map && value.containsKey('amount')) {
              // Structured format with amount field
              double amount = value['amount'] is num
                  ? (value['amount'] as num).toDouble()
                  : _extractNumericValue(value['amount'].toString());

              if (amount > 0) {
                _updateVitaminWithValue(vitaminKey, amount);
                print(
                    'Found structured $vitaminKey: $amount from vitamins nested data');
              }
            } else {
              // Direct value format
              ...
              if (amount > 0) {
                _updateVitaminWithValue(vitaminKey, amount);
                print('Found direct $vitaminKey: $amount from vitamins data');
              }
            }
          }
        });
      ...
      print('Error processing structured vitamins data: $e');
    }

    // Process structured 'minerals' field
    try {
      if (data.containsKey('minerals') && data['minerals'] is Map) {
        Map<String, dynamic> mineralsData =
            Map<String, dynamic>.from(data['minerals']);

        // Direct mineral mapping
        Map<String, String> apiToUiMineralMap = {
          'sodium': 'Sodium',
          'calcium': 'Calcium',
          'iron': 'Iron',
          'potassium': 'Potassium',
        ...
        mineralsData.forEach((apiKey, value) {
          String mineralKey = apiToUiMineralMap[apiKey.toLowerCase()] ?? apiKey;
          ...
          if (minerals.containsKey(mineralKey)) {
            if (value is Map && value.containsKey('amount')) {
              ...
              if (amount > 0) {
                ...
                print(
                    'Found structured $mineralKey: $amount from minerals nested data');
              }
            } else {
              // Direct value format
              double amount = _extractNumericValue(value.toString());
              if (amount > 0) {
                _updateMineralWithValue(mineralKey, amount);
                print('Found direct $mineralKey: $amount from minerals data');
              }
            }
          }
        });
      ...
      print('Error processing structured minerals data: $e');
    }

    // Process structured 'other_nutrients' field or 'fiber'/'cholesterol' fields
    try {
      // Check for other_nutrients structured data
      if (data.containsKey('other_nutrients') &&
          data['other_nutrients'] is Map) {
        Map<String, dynamic> otherData =
            Map<String, dynamic>.from(data['other_nutrients']);

        // Map from API keys to UI keys
        Map<String, String> apiToUiOtherMap = {
          'fiber': 'Fiber',
        ...
        otherData.forEach((apiKey, value) {
          String nutrientKey = apiToUiOtherMap[apiKey.toLowerCase()] ?? apiKey;
          ...
          if (other.containsKey(nutrientKey)) {
            if (value is Map && value.containsKey('amount')) {
              ...
              if (amount > 0) {
                ...
                print(
                    'Found structured $nutrientKey: $amount from other_nutrients nested data');
              }
            } else {
              // Direct value format
              double amount = _extractNumericValue(value.toString());
              if (amount > 0) {
                _updateOtherNutrientWithValue(nutrientKey, amount);
                print(
                    'Found direct $nutrientKey: $amount from other_nutrients data');
              }
            }
          }
        });
      ...
      // Check for fiber, cholesterol as structured objects
      if (data.containsKey('fiber') &&
          data['fiber'] is Map &&
          data['fiber'].containsKey('amount')) {
        double amount = data['fiber']['amount'] is num
            ? (data['fiber']['amount'] as num).toDouble()
            : _extractNumericValue(data['fiber']['amount'].toString());
        if (amount > 0) {
          _updateOtherNutrientWithValue('Fiber', amount);
          print('Found structured Fiber: $amount');
        }
      }

      if (data.containsKey('cholesterol') &&
          data['cholesterol'] is Map &&
          data['cholesterol'].containsKey('amount')) {
        double amount = data['cholesterol']['amount'] is num
            ? (data['cholesterol']['amount'] as num).toDouble()
            : _extractNumericValue(data['cholesterol']['amount'].toString());
        if (amount > 0) {
          _updateOtherNutrientWithValue('Cholesterol', amount);
          print('Found structured Cholesterol: $amount');
        }
      }
    } catch (e) {
      print('Error processing structured other nutrients data: $e');
    }

    // THEN process values directly in the root object AFTER processing nested structures
  ...
  void _updateVitaminWithValue(String vitaminKey, double currentAmount) {
    if (vitamins.containsKey(vitaminKey)) {
      ...
      // Extract the existing amount and add the new amount to it
      double existingAmount = _parseCurrentValue(currentValue);
      double totalAmount = existingAmount + currentAmount;

      // Round to exactly one decimal place
      totalAmount = double.parse(totalAmount.toStringAsFixed(1));

      print(
          'NUTRIENT ACCUMULATION: $vitaminKey - Adding $currentAmount to existing $existingAmount = $totalAmount $unit');
      ...
      _loadTargetValueFromPrefs(
        ...
        double progress = targetValue > 0 ? (totalAmount / targetValue) : 0;
        ...
        if (mounted) {
          setState(() {
            vitamins[vitaminKey] = NutrientInfo(
              name: vitaminKey,
                value: "$totalAmount/${targetValue.toStringAsFixed(1)} $unit",
              percent: "$percentage%",
              progress: progress,
                progressColor: progressColor);
          });
        ...
        print(
            'Updated $vitaminKey: $totalAmount/$targetValue $unit = $percentage%');
      });
    }
  }
  ...
  void _updateMineralWithValue(String mineralKey, double currentAmount) {
    if (minerals.containsKey(mineralKey)) {
      ...
      print(
          'NUTRIENT ACCUMULATION: $mineralKey - Adding $currentAmount to existing $existingAmount = $totalAmount $unit');
      ...
      _loadTargetValueFromPrefs(
        ...
        print(
            'Updated $mineralKey: $totalAmount/$targetValue $unit = $percentage%');
      });
    }
  }
  ...
  void _updateOtherNutrientWithValue(String nutrientKey, double currentAmount) {
    if (other.containsKey(nutrientKey)) {
      ...
      print(
          'NUTRIENT ACCUMULATION: $nutrientKey - Adding $currentAmount to existing $existingAmount = $totalAmount $unit');
      ...
      _loadTargetValueFromPrefs(nutrientKey).then((targetValue) {
        ...
        print(
            'Updated $nutrientKey: $totalAmount/$targetValue $unit = $percentage%');
      });
    }
  }
