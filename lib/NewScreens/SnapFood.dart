...
class _SnapFoodState extends State<SnapFood> {
  ...
  void _displayAnalysisResults(
    ...
    try {
      ...
      if (analysisData.containsKey('meal_name')) {
        ...
        // Extract top-level micronutrients for distribution to ingredients
        Map<String, dynamic> topLevelVitamins = {};
        Map<String, dynamic> topLevelMinerals = {};
        Map<String, dynamic> topLevelOtherNutrients = {};

        // Process top-level vitamins
        ...
        // Process top-level other nutrients
        if ((analysisData.containsKey('other') &&
                analysisData['other'] is Map) ||
            (analysisData.containsKey('other_nutrients') &&
                analysisData['other_nutrients'] is Map)) {
          topLevelOtherNutrients = Map<String, dynamic>.from(
              analysisData['other'] ?? analysisData['other_nutrients'] ?? {});
        }
        ...
        for (int i = 0; i < ingredients.length; i++) {
          ...
          if (i < ingredientMacros.length && ingredientMacros[i] is Map) {
            ...
            // Check for vitamins, minerals and other nutrients
            ...
            // If still no micronutrients found, add the top-level micronutrients to each ingredient
            // This is the key improvement - always add top-level nutrients to each ingredient
            if (topLevelVitamins.isNotEmpty &&
                (vitamins.isEmpty || vitamins.length < 3)) {
              _extractNutrientValues(topLevelVitamins, vitamins);
            }

            if (topLevelMinerals.isNotEmpty &&
                (minerals.isEmpty || minerals.length < 3)) {
              _extractNutrientValues(topLevelMinerals, minerals);
            }

            if (topLevelOtherNutrients.isNotEmpty && other.isEmpty) {
              _extractNutrientValues(topLevelOtherNutrients, other);
            }

            // Add the micronutrient data to the ingredient with proper units
            if (vitamins.isNotEmpty) {
              Map<String, dynamic> vitaminData = {};
              vitamins.forEach((name, value) {
                vitaminData[name] = {
                  'amount': value,
                  'unit': _getUnitForVitamin(name)
                };
              });
              ingredientData['vitamins'] = vitaminData;
            ...
            if (minerals.isNotEmpty) {
              ...
              minerals.forEach((name, value) {
                mineralData[name] = {
                  'amount': value,
                  'unit': _getUnitForMineral(name)
                };
              });
              ingredientData['minerals'] = mineralData;
            ...
            if (other.isNotEmpty) {
              ...
              other.forEach((name, value) {
                otherData[name] = {
                  'amount': value,
                  'unit': _getUnitForNutrient(name)
                };
              });
              ingredientData['other_nutrients'] = otherData;
            ...
            // Add top-level micronutrients to each ingredient
            if (topLevelVitamins.isNotEmpty) {
              Map<String, double> vitamins = {};
              _extractNutrientValues(topLevelVitamins, vitamins);
              ...
              if (vitamins.isNotEmpty) {
                ...
                print(
                    '\nIngredient: ${ingredientData['name']} - Using global vitamin data');
                vitamins.forEach((name, value) {
                  print('    • $name: $value${_getUnitForVitamin(name)}');
                });
              }
            ...
            if (topLevelMinerals.isNotEmpty) {
              ...
              if (minerals.isNotEmpty) {
                ...
                print('  Using global mineral data');
                minerals.forEach((name, value) {
                  print('    • $name: $value${_getUnitForMineral(name)}');
                });
              }
            ...
            if (topLevelOtherNutrients.isNotEmpty) {
              Map<String, double> other = {};
              _extractNutrientValues(topLevelOtherNutrients, other);
              ...
              if (other.isNotEmpty) {
                ...
                print('  Using global other nutrient data');
                other.forEach((name, value) {
                  print('    • $name: $value${_getUnitForNutrient(name)}');
                });
              }
  ...
  void _extractNutrientValues(
    ...
    source.forEach((key, value) {
      ...
      // Only add values >= 0.01 (more inclusive to capture small but important nutrient amounts)
      if (numValue >= 0.01) {
        target[key] = numValue;
      }
    });
  ...
  String _getUnitForVitamin(String vitaminName) {
    ...
    if (vitaminName.contains('a') ||
        vitaminName.contains('d') ||
        vitaminName.contains('k') ||
        vitaminName.contains('b7') ||
        vitaminName.contains('b9') ||
        vitaminName.contains('b12') ||
        vitaminName.contains('folate') ||
        vitaminName.contains('biotin')) return 'mcg'; // micrograms

    // Default for most vitamins (B1, B2, B3, B5, B6, C, E)
  ...
  String _getUnitForMineral(String mineralName) {
    ...
    if (mineralName.contains('selenium') ||
        ...
        mineralName.contains('iodine') ||
        mineralName.contains('molybdenum')) return 'mcg';

    // These minerals are commonly measured in micrograms but API sometimes returns in mg
    if (mineralName.contains('copper') || mineralName.contains('fluoride'))
      return 'mcg';

    // Default for most minerals (calcium, potassium, iron, etc.)
    return 'mg';
  }
  ...
  String _getUnitForNutrient(String nutrientName) {
    ...
    // Milligrams
    if (nutrientName.contains('cholesterol') ||
        nutrientName.contains('sodium') ||
        nutrientName.contains('omega-3') ||
        nutrientName.contains('omega3')) return 'mg';

    // Grams (fiber, sugars, fats)
    if (nutrientName.contains('fiber') ||
        nutrientName.contains('sugar') ||
        nutrientName.contains('fat') ||
        nutrientName.contains('omega-6') ||
        nutrientName.contains('omega6')) return 'g';
  ...
  Map<String, dynamic> _extractAdditionalNutrients(
    ...
    if (analysisData.containsKey('vitamins') &&
      ...
      for (final entry in vitamins.entries) {
        ...
        // Add unit information for better display in Nutrition screen
        if (value is Map &&
            value.containsKey('amount') &&
            value.containsKey('unit')) {
          // Already has structured data with amount and unit
          nutrients[normalizedKey] = value;
        } else {
          // Create structured data with amount and appropriate unit
          ...
          nutrients[normalizedKey] = {'amount': amount, 'unit': unit};
        }
      }
    }

    // Direct extraction of minerals with proper units
    if (analysisData.containsKey('minerals') &&
        analysisData['minerals'] is Map) {
    ...
    // Direct extraction of other nutrients with proper units
    if ((analysisData.containsKey('other_nutrients') &&
            analysisData['other_nutrients'] is Map) ||
        (analysisData.containsKey('other') && analysisData['other'] is Map)) {
      final Map<String, dynamic> otherNutrients = Map<String, dynamic>.from(
          analysisData['other_nutrients'] ?? analysisData['other'] ?? {});
    ...
    if (analysisData.containsKey('ingredient_macros') &&
      ...
      for (var macroData in ingredientMacros) {
        if (macroData is Map) {
          ...
          // Process vitamins from this ingredient's data
          if (macros.containsKey('vitamins') && macros['vitamins'] is Map) {
            Map<String, dynamic> vitaminData =
                Map<String, dynamic>.from(macros['vitamins']);
            vitaminData.forEach((key, value) {
              String normalizedKey = key.toLowerCase();

              // Normalize vitamin keys
              if (normalizedKey.length <= 3 &&
                  (normalizedKey == 'a' ||
                      normalizedKey == 'c' ||
                      normalizedKey == 'd' ||
              ...
              // Only add if not already present with higher value
              if (!nutrients.containsKey(normalizedKey) ||
                  _getNutrientAmount(nutrients[normalizedKey]) <
                      _getNutrientAmount(value)) {
                if (value is Map && value.containsKey('amount')) {
                  nutrients[normalizedKey] = value;
                } else {
                  // Add with appropriate unit
                  String unit = _getUnitForVitamin(normalizedKey);
                  double amount = 0.0;
          ...
          // Process minerals from this ingredient's data
          if (macros.containsKey('minerals') && macros['minerals'] is Map) {
            Map<String, dynamic> mineralData =
                Map<String, dynamic>.from(macros['minerals']);
    ...
    for (final nutrient in commonNutrients) {
      if (analysisData.containsKey(nutrient)) {
        String unit = nutrient.startsWith('vitamin_')
            ? _getUnitForVitamin(nutrient)
            : (commonMinerals.contains(nutrient)
                ? _getUnitForMineral(nutrient)
                : _getUnitForNutrient(nutrient));

        double amount = 0.0;
        var value = analysisData[nutrient];
        ...
        nutrients[nutrient] = {'amount': amount, 'unit': unit};
      }
    }
  ...
  // Helper to get the numeric amount from a nutrient value, whether formatted or raw
  double _getNutrientAmount(dynamic value) {
  ...
  // List of common minerals for unit determination
  final List<String> commonMinerals = [
    'calcium',
    'iron',
    'magnesium',
    'phosphorus',
  ...
  void _extractNestedNutrients(
    ...
    if (source.containsKey('vitamins') && source['vitamins'] is Map) {
      ...
      vitamins.forEach((key, value) {
        ...
        if (!target.containsKey(normalizedKey) ||
            _getNutrientAmount(target[normalizedKey]) <
                _getNutrientAmount(value)) {
          // Handle different value types with proper unit
          if (value is Map && value.containsKey('amount')) {
            if (!value.containsKey('unit')) {
              value['unit'] = _getUnitForVitamin(normalizedKey);
            }
            ...
            target[normalizedKey] = {
              'amount': amount,
              'unit': _getUnitForVitamin(normalizedKey)
            };
          }
        }
      });
    ...
    // Check for minerals with similar approach
    if (source.containsKey('minerals') && source['minerals'] is Map) {
      ...
      minerals.forEach((key, value) {
        ...
        if (!target.containsKey(normalizedKey) ||
          ...
          if (value is Map && value.containsKey('amount')) {
            if (!value.containsKey('unit')) {
              value['unit'] = _getUnitForMineral(normalizedKey);
            }
            target[normalizedKey] = value;
          } else {
            double amount = 0.0;
            ...
            target[normalizedKey] = {
              'amount': amount,
              'unit': _getUnitForMineral(normalizedKey)
            };
          }
        }
      });
    ...
    // Check for other nutrients with similar approach
    if (source.containsKey('other') && source['other'] is Map) {
      ...
      other.forEach((key, value) {
        ...
        if (!target.containsKey(normalizedKey) ||
          ...
          if (value is Map && value.containsKey('amount')) {
            if (!value.containsKey('unit')) {
              value['unit'] = _getUnitForNutrient(normalizedKey);
            }
            target[normalizedKey] = value;
          } else {
            double amount = 0.0;
            ...
            target[normalizedKey] = {
              'amount': amount,
              'unit': _getUnitForNutrient(normalizedKey)
            };
          }
        }
      });
      ...
      if (key != 'vitamins' && key != 'minerals' && key != 'other') {
        ...
        if (!target.containsKey(normalizedKey) ||
          ...
          if (normalizedKey.startsWith('vitamin_')) {
            unit = _getUnitForVitamin(normalizedKey);
          } else if (commonMinerals.contains(normalizedKey)) {
            unit = _getUnitForMineral(normalizedKey);
          } else {
            unit = _getUnitForNutrient(normalizedKey);
          }
          ...
          if (value is Map && value.containsKey('amount')) {
            ...
            target[normalizedKey] = {'amount': amount, 'unit': unit};
          }
        }
      }
    });
